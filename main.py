...
async def check_winner(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    async with aiosqlite.connect("bot.db") as db:
        async with db.execute(
            "SELECT wins, defeats FROM users WHERE user_id = ?", (user_id,)
        ) as cursor:
            ...
            if row:
                ...
                if is_winner:
                    w = row[0] + 1 if row[0] is not None else 1
                    await db.execute(
                        """
                        UPDATE users
                    ...
                    d = row[1] + 1 if row[1] is not None else 1
                    await db.execute(
                        """
                        UPDATE users
...
async def choosing_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if int(query.data) == PLAY:
        ...
        context.job_queue.run_once(
            once_job_step, 10, chat_id=user_id, name=f"{user_id}-{ONCE}"
        )
        ...
        async with aiosqlite.connect("bot.db") as db:
            async with db.execute(
                "SELECT name FROM users WHERE user_id = ?", (update.effective_user.id,)
            ) as cursor:
                row = await cursor.fetchone()
...
async def save_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        "Please, send me your phone number:", reply_markup=reply_markup
    )
    return PHONE
...
async def save_phone(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    async with aiosqlite.connect("bot.db") as db:
        async with db.execute(
            "SELECT conversation_status FROM users WHERE user_id = ?", (user_id,)
        ) as cursor:
...
async def get_users(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    async with aiosqlite.connect("bot.db") as db:
        async with db.execute(
            "SELECT user_id, wins, defeats, name, phone, conversation_status FROM users"
        ) as cursor:
    ...
    if len(messages) != 0:
        await update.message.reply_text(
            "user_id: (wins - defeats) - name - phone #conversationStatus"
        )
    ...
    async with aiosqlite.connect("bot.db") as db:
        async with db.execute("SELECT user_id FROM users") as cursor:
            async for row in cursor:
                await context.bot.send_message(
                    chat_id=row[0], text=msg, parse_mode="MarkdownV2"
                )
    ...
    async with aiosqlite.connect("bot.db") as db:
        async with db.execute("SELECT user_id, name FROM users") as cursor:
            ...
            for row in rows:
                ...
                if row[1]:
                    await context.bot.send_message(
                        chat_id=row[0], text=f"Hello, {row[1]}, let's play!"
                    )
                else:
                    await context.bot.send_message(
                        chat_id=row[0], text="Hello, let's play!"
                    )
...
def main() -> None:
    ...
    application.job_queue.run_daily(
        daily_job_progrev,
        time=time(hour=17, minute=54, tzinfo=pytz.timezone("Europe/Moscow")),
    )
